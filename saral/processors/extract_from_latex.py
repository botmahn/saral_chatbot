...
class LatexProjectParser:
    """
    Parser to convert LaTeX projects (e.g., arXiv-style papers) into JSON:
      - main text (with sections/subsections)
      - images metadata
      - equations metadata
      - tables metadata
    """
    ...
    def clean_text_block(self, text: Optional[str]) -> str:
        """
        ...
        text = re.sub(
            r'\\(textbf|textit|emph|texttt|textsc|sc|bf|it)\{([^}]+)\}',
            r'\2',
            text,
            flags=re.IGNORECASE,
        )
        ...
        text = re.sub(
            r'~?\\(cite|citep|citet|ref|label|url|footnote|thanks)\[[^\]]*?\]\{[^}]+\}',
            '',
            text,
            flags=re.IGNORECASE,
        )
    ...
    def resolve_inputs(self, tex_content: str, base_dir: str) -> str:
        """
        ...
        def replacer(match):
            ...
            if os.path.exists(filepath):
                try:
                    ...
                    return self.resolve_inputs(content, os.path.dirname(filepath))
    ...
    # -------------- Metadata Extraction (figures, equations, tables) -----------------
    ...
    def extract_metadata(
        ...
        for fig_block in figure_pattern.finditer(full_content):
            ...
            if cap_match:
                image_meta["caption"] = self.clean_text_block(cap_match.group(1))
        ...
        for tbl_block in table_pattern.finditer(full_content):
            ...
            cap_match = caption_pattern.search(block_text)
            if cap_match:
                table_meta["caption"] = self.clean_text_block(cap_match.group(1))
    ...
    def parse_tex_to_json(
        self, main_tex_path: str
    ) -> Tuple[Optional[dict], Optional[List[dict]], Optional[List[dict]], Optional[List[dict]]]:
        """
        Parse a LaTeX project starting from main_tex_path.

        Returns:
            text_data, image_data, equation_data, table_data
        """
        ...
        if self.verbose:
            print(f"[INFO] Resolving inputs for {main_tex_path} ...")

        # 1. Resolve all \input / \include commands
        full_content = self.resolve_inputs(initial_content, base_dir)

        # 2. Extract metadata BEFORE cleaning
        image_data, equation_data, table_data = self.extract_metadata(full_content)
        ...
        # 4. Extract title, author, abstract
        try:
            ...
            if title_match:
                data['title'] = self.clean_text_block(title_match.group(1))
        except Exception as e:
        ...
        try:
            ...
            if author_match:
                ...
                data['author'] = self.clean_text_block(author_text)
        except Exception as e:
        ...
        try:
            ...
            if abstract_match:
                data['abstract'] = self.clean_text_block(abstract_match.group(1))
        except Exception as e:
        ...
        if not main_content_match:
            print(
                "Error: Could not find \\begin{document}...\\end{document} block.",
                file=sys.stderr,
            )
        ...
        intro_text = self.clean_text_block(parts[0])
        if intro_text:
            data['content'].append(
                {
                    "level": 0,
        ...
        # Groups of (section_type, title, text)
    ...
    @staticmethod
    def _derive_output_paths(base_output_path: str) -> Tuple[str, str, str, str]:
        """
        Given a base output path (without extension or with .json),
        derive the four JSON output paths.
        """
        ...
        return text_output_file, image_output_file, equation_output_file, table_output_file

    def parse_and_save(self, main_tex_path: str, base_output_path: str) -> None:
        """
        Convenience method: parse a single LaTeX project and save all 4 JSONs.
        """
        if self.verbose:
            print(f"Starting parsing of {main_tex_path}...")

        text_output_file, image_output_file, equation_output_file, table_output_file = (
            self._derive_output_paths(base_output_path)
        )

        text_data, image_data, equation_data, table_data = self.parse_tex_to_json(
            main_tex_path
        )
    ...
    def parse_many(self, main_tex_files: List[str], output_dir: str) -> None:
        """
        Parse and save JSONs for multiple LaTeX projects.

        Args:
            main_tex_files: list of paths to main .tex files.
            output_dir: directory where JSONs will be stored.
                        Each paper gets files named <basename>.json,
                        <basename>_images.json, etc.
        """
        ...
        for tex_path in main_tex_files:
            ...
            if self.verbose:
                print(f"\n[INFO] Processing paper: {tex_path}")
                print(f"[INFO] Base output path: {base_output_path}")
            self.parse_and_save(tex_path, base_output_path)
...
def main():
    parser = argparse.ArgumentParser(
        description='Convert arXiv-style LaTeX projects into clean JSON files.'
    )
    parser.add_argument(
        'main_tex_files',
        type=str,
        nargs='+',
        help='Path(s) to the main .tex files of the project(s).',
    )
    parser.add_argument(
        'output_dir',
        type=str,
        help='Directory to save the output JSON files for all papers.',
    )
    parser.add_argument(
        '--quiet',
        action='store_true',
    ...
    lp_parser = LatexProjectParser(verbose=not args.quiet)
    lp_parser.parse_many(args.main_tex_files, args.output_dir)
